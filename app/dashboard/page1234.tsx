...
// Platform Icons - Monoline style matching site design
const TwitterIcon = () => (
  <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
...
const LinkedInIcon = () => (
  <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
    ...
    <path strokeLinecap="round" strokeLinejoin="round" d="M7 10v7M7 7v.01M11 17v-4a2 2 0 014 0v4M11 10v7" />
  </svg>
)
...
const languageConfig: Record<string, { name: string; flag: string; rtl: boolean }> = {
  english: { name: 'English', flag: '🇬🇧', rtl: false },
  spanish: { name: 'Español', flag: '🇪🇸', rtl: false },
  portuguese: { name: 'Português', flag: '🇧🇷', rtl: false },
  french: { name: 'Français', flag: '🇫🇷', rtl: false },
  arabic: { name: 'العربية', flag: '🇸🇦', rtl: true },
  hindi: { name: 'हिन्दी', flag: '🇮🇳', rtl: false }
}
...
export default function Dashboard() {
  ...
  const handlePlatformChange = (newPlatform: string) => {
    if (generatedContent && newPlatform !== originalPlatform) {
  ...
  const handleShare = async (p: string) => {
    ...
    const urls: Record<string, string> = {
      twitter: `https://twitter.com/intent/tweet?text=${encodeURIComponent(generatedContent)}`,
      facebook: `https://www.facebook.com/sharer/sharer.php?quote=${encodeURIComponent(generatedContent)}`,
      linkedin: `https://www.linkedin.com/sharing/share-offsite/?summary=${encodeURIComponent(generatedContent)}`
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#FDFAFF] to-[#F3E8FF] flex flex-col">
      {/* Header */}
      <header className="border-b border-gray-200 bg-white/80 backdrop-blur-sm sticky top-0 z-50">
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => setShowSettings(!showSettings)} className="p-2 text-gray-600 hover:text-[#6B2C6B] transition-colors" title="Settings">
              <CogIcon />
            </button>
            <div className="w-8 h-8 bg-[#6B2C6B] rounded-full flex items-center justify-center text-white font-bold text-sm">
              {user?.email?.[0].toUpperCase() || 'U'}
            </div>
            <button onClick={signOut} className="p-2 text-gray-600 hover:text-[#6B2C6B] transition-colors" title="Sign Out">
              <LogOutIcon />
            </button>
          </div>
        </div>
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-6 py-8">
        ...
        {showSettings && (
          <div className="mb-6 bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2"><CogIcon /> Account Settings</h3>
            ...
            <button onClick={signOut} className="px-4 py-2 bg-[#6B2C6B] text-white rounded-lg hover:bg-[#8B3A8B] transition-colors">Sign Out</button>
          </div>
        )}

        {/* History Panel */}
        {showHistory && (
          <div className="mb-6 bg-white rounded-2xl p-6 shadow-sm border border-gray-100 max-h-80 overflow-y-auto">
            <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2"><ClockIcon /> Recent Posts</h3>
            {postHistory.length === 0 ? (
              <p className="text-gray-500">No posts yet. Generate your first post!</p>
            ) : (
        ...
        <div className="mb-6 bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-2 gap-4">
            <button
              onClick={() => handlePostTypeChange('product')}
              className={`p-6 rounded-xl border-2 transition-all ${postType === 'product' ? 'border-[#6B2C6B] bg-[#F3E8FF]' : 'border-gray-200 hover:border-[#6B2C6B]/50'}`}
            >
              <CameraIcon className={postType === 'product' ? 'text-[#6B2C6B]' : 'text-gray-400'} />
              <p className="font-bold text-gray-800 mt-2">Product Post</p>
              <p className="text-sm text-gray-500">Upload an image</p>
            </button>
            <button
              onClick={() => handlePostTypeChange('text')}
              className={`p-6 rounded-xl border-2 transition-all ${postType === 'text' ? 'border-[#6B2C6B] bg-[#F3E8FF]' : 'border-gray-200 hover:border-[#6B2C6B]/50'}`}
            >
              <PencilIcon className={postType === 'text' ? 'text-[#6B2C6B]' : 'text-gray-400'} />
              <p className="font-bold text-gray-800 mt-2">Text Post</p>
              <p className="text-sm text-gray-500">Describe your content</p>
            </button>
          </div>
        </div>
            ...
            {postType === 'product' ? (
              <>
                ...
                <div className="mb-4">
                  ...
                  <div className="border-2 border-dashed border-gray-200 rounded-xl p-8 text-center hover:border-[#6B2C6B]/50 transition-colors bg-gray-50">
                    {imagePreview ? (
                      <div className="relative inline-block">
                        <img src={imagePreview} alt="Preview" className="max-h-48 rounded-lg shadow-md" />
                        <button onClick={() => { setImagePreview(null); setImageCheckStatus('idle') }} className="absolute -top-2 -right-2 bg-[#6B2C6B] text-white p-1.5 rounded-full hover:bg-[#8B3A8B]"><XIcon /></button>
                      </div>
                    ) : (
                      <>
                        <UploadIcon className="mx-auto text-gray-300 mb-3" />
                        <label className="cursor-pointer text-[#6B2C6B] font-semibold hover:underline">
                          Click to upload
                ...
                {imageCheckStatus === 'checking' && (
                  <div className="mb-4 p-3 bg-[#F3E8FF] rounded-xl flex items-center gap-2">
                    <LoaderIcon className="w-4 h-4 text-[#6B2C6B]" />
                    <span className="text-sm text-[#6B2C6B]">Checking image safety...</span>
                  </div>
                )}
                {imageCheckStatus === 'passed' && (
                  <div className="mb-4 p-3 bg-green-50 rounded-xl flex items-center gap-2">
                    <ShieldCheckIcon />
                    <span className="text-sm text-green-700">Image verified</span>
                  </div>
                )}
                {imageCheckStatus === 'failed' && (
                ...
                {category === 'books' && (
                  <div className="mb-4 p-4 bg-[#F3E8FF] rounded-xl space-y-3">
                    <div>
                      <label className="block text-sm font-semibold text-[#6B2C6B] mb-1">Book Title</label>
                      <input value={bookTitle} onChange={(e) => setBookTitle(e.target.value)} placeholder="Enter book title" className="w-full p-2 border-2 border-[#6B2C6B]/20 rounded-lg" />
                    </div>
                    <div>
                      <label className="block text-sm font-semibold text-[#6B2C6B] mb-1">Author</label>
                      <input value={bookAuthor} onChange={(e) => setBookAuthor(e.target.value)} placeholder="Enter author name" className="w-full p-2 border-2 border-[#6B2C6B]/20 rounded-lg" />
                    </div>
                  </div>
                )}
                ...
                <div className="mb-4">
                  ...
                  <input value={brandName} onChange={(e) => setBrandName(e.target.value)} placeholder="Brand name" className="w-full p-3 border-2 border-gray-200 rounded-xl" />
                </div>
              </>
            ) : (
              <>
                ...
                <div className="mb-4">
                  <label className="block text-sm font-semibold text-gray-700 mb-2">Content <span className="text-[#6B2C6B]">*</span></label>
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">Max {platformConfig[platform]?.charLimit.toLocaleString()}</p>
                  </div>
                  <div>
                ...
                {showPlatformWarning && (
                  <div className="mb-4 p-3 bg-[#F3E8FF] border border-[#6B2C6B]/20 rounded-xl flex items-start gap-2">
                    ...
                    <div>
                      <p className="text-sm text-[#6B2C6B] font-semibold">Platform changed</p>
                      <p className="text-xs text-[#8B3A8B]">Content was optimized for {platformConfig[originalPlatform]?.name}. Click "Regenerate" for best results on {platformConfig[platform]?.name}.</p>
                    </div>
                  </div>
                )}
              ...
              <div className="flex flex-col flex-1">
                ...
                <div className="mb-4 flex items-center justify-between flex-shrink-0">
                  ...
                  <div className="text-sm">
                    <span className={`font-semibold ${isOverLimit ? 'text-[#6B2C6B]' : isNearLimit ? 'text-[#8B3A8B]' : 'text-gray-500'}`}>
                      {charCount.toLocaleString()}
                    </span>
                    <span className="text-gray-400">/{charLimit.toLocaleString()}</span>
                  </div>
                </div>
                ...
                <div className="mt-4 flex-shrink-0">
                  ...
                  >
                    ...
                    {platform === 'instagram' || platform === 'tiktok' 
                      ? (copySuccess ? `✓ Copied! Opening ${platformConfig[platform]?.name}...` : `Copy & Open ${platformConfig[platform]?.name}`)
                      : `Share to ${platformConfig[platform]?.name}`
                    }
                  </button>
                </div>
              </div>
