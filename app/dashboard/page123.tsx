...
export default function Dashboard() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const body: any = { postType, platform, mood, features: selectedFeatures, charLimit }
  ...
  const handleShare = async (shareType: string) => {
    if (shareType === 'twitter') {
      window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(generatedContent)}`, '_blank', 'width=550,height=420')
    } else if (shareType === 'facebook') {
      window.open(`https://www.facebook.com/sharer/sharer.php?quote=${encodeURIComponent(generatedContent)}`, '_blank', 'width=550,height=420')
    } else if (shareType === 'linkedin') {
      window.open(`https://www.linkedin.com/shareArticle?mini=true&summary=${encodeURIComponent(generatedContent)}`, '_blank', 'width=550,height=420')
    } else if (shareType === 'instagram' || shareType === 'tiktok') {
      await navigator.clipboard.writeText(generatedContent)
      setCopySuccess(true)
      setTimeout(() => {
        setCopySuccess(false)
        window.open(shareType === 'instagram' ? 'https://www.instagram.com/' : 'https://www.tiktok.com/', '_blank')
      }, 1500)
    }
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#FDFAFF] via-[#FDF2F8] to-[#FFF8F8] flex flex-col">
              ...
              postHistory.map((post, idx) => (
                <div key={idx} className="mb-4 p-3 bg-gray-50 rounded-lg">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-xs font-semibold text-[#6B2C6B] capitalize">{post.platform}</span>
      ...
      {showSettings && (
        <div className="fixed right-0 top-0 h-full w-80 bg-white shadow-2xl z-50 border-l border-gray-100">
          ...
          <div className="p-6">
            <div className="mb-6 p-4 bg-[#F3E8FF] rounded-xl">
              <p className="font-bold text-[#6B2C6B]">{user?.email}</p>
              <p className="text-sm text-[#8B3A8B] mt-1">Free Plan - 60 posts/month</p>
            </div>
            <button onClick={signOut} className="w-full p-3 bg-gray-100 text-gray-700 rounded-xl font-semibold flex items-center justify-center gap-2 hover:bg-gray-200">
              <LogOutIcon /> Sign Out
            </button>
          </div>
        </div>
      ...
      <main className="max-w-5xl mx-auto px-6 py-8 flex-1">
        {/* Post Type Selection - Top Cards */}
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 mb-6">
          <h3 className="font-semibold text-[#6B2C6B] mb-4">What would you like to create?</h3>
          <div className="grid md:grid-cols-2 gap-4">
            <button 
              onClick={() => handlePostTypeChange('product')} 
              className={`p-6 rounded-xl border-2 text-left transition-all ${postType === 'product' ? 'border-[#6B2C6B] bg-[#F3E8FF]' : 'border-gray-200 hover:border-gray-300'}`}
            >
              <CameraIcon className="w-8 h-8 text-[#6B2C6B] mb-3" />
              <p className="font-bold text-gray-800">Product Post</p>
              <p className="text-sm text-gray-500">Upload an image</p>
            </button>
            <button 
              onClick={() => handlePostTypeChange('text')} 
              className={`p-6 rounded-xl border-2 text-left transition-all ${postType === 'text' ? 'border-[#6B2C6B] bg-[#F3E8FF]' : 'border-gray-200 hover:border-gray-300'}`}
            >
              <PencilIcon className="w-8 h-8 text-[#6B2C6B] mb-3" />
              <p className="font-bold text-gray-800">Text Post</p>
        ...
        <div className="grid md:grid-cols-2 gap-6 items-stretch">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex flex-col justify-between min-h-[550px]">
            ...
            <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2">
              {postType === 'product' ? <><CameraIcon className="w-5 h-5" /> Product Details</> : <><PencilIcon className="w-5 h-5" /> Text Details</>}
            </h3>
              ...
              <>
                  ...
                  <div className={`border-2 border-dashed rounded-xl p-4 text-center transition-all ${imageCheckStatus === 'passed' ? 'border-[#6B2C6B] bg-[#F3E8FF]/30' : 'border-gray-200 hover:border-gray-300'}`}>
                    {imagePreview ? (
                      <div className="relative inline-block">
                        ...
                        <button onClick={() => { setImagePreview(null); setImageCheckStatus('idle'); setCategory(''); }} className="absolute -top-2 -right-2 p-1 bg-[#6B2C6B] text-white rounded-full hover:bg-[#5a245a]">
                          <XIcon />
                        </button>
                        {imageCheckStatus === 'passed' && (
                          <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 px-2 py-0.5 bg-[#6B2C6B] text-white text-xs rounded-full flex items-center gap-1">
                      ...
                      <div className="py-2">
                        <LoaderIcon className="w-6 h-6 mx-auto text-[#6B2C6B]" />
                        <p className="text-xs text-gray-500 mt-1">Checking...</p>
                      </div>
                    ) : (
                      <label className="cursor-pointer block py-2">
                        <UploadIcon className="mx-auto text-gray-400 mb-1 w-8 h-8" />
                        <span className="text-sm text-[#6B2C6B] font-medium">Click to upload</span>
                ...
                <div className="mb-4">
                  ...
                  <select 
                    ...
                    disabled={imageCheckStatus !== 'passed'}
                    className={`w-full p-3 border-2 border-gray-200 rounded-xl focus:border-[#6B2C6B] outline-none ${imageCheckStatus !== 'passed' ? 'bg-gray-50 text-gray-400' : ''}`}
                  >
                ...
                {/* Brand - Always visible (changes to Book fields if books selected) */}
                <div className="mb-4">
                  {category === 'books' ? (
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-2">Book Title</label>
                        <input type="text" value={bookTitle} onChange={(e) => setBookTitle(e.target.value)} placeholder="Title" className="w-full p-3 border-2 border-gray-200 rounded-xl" />
                      </div>
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-2">Author</label>
                        <input type="text" value={bookAuthor} onChange={(e) => setBookAuthor(e.target.value)} placeholder="Author" className="w-full p-3 border-2 border-gray-200 rounded-xl" />
                      </div>
                    </div>
                  ) : (
                    <>
                      ...
                      <input 
                        ...
                        disabled={imageCheckStatus !== 'passed'}
                        className={`w-full p-3 border-2 border-gray-200 rounded-xl ${imageCheckStatus !== 'passed' ? 'bg-gray-50 text-gray-400' : ''}`}
                      />
                    </>
                  )}
                </div>
